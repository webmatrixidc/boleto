...
namespace WebMatrixIDC\Boleto\Cnab\Retorno\Cnab240\Banco;

use WebMatrixIDC\Boleto\Cnab\Retorno\Cnab240\AbstractRetorno;
use WebMatrixIDC\Boleto\Contracts\Boleto\Boleto as BoletoContract;
use WebMatrixIDC\Boleto\Contracts\Cnab\RetornoCnab240;
    ...
    protected function processarDetalhe(array $detalhe)
    {
        ...
        if ($this->getSegmentType($detalhe) == 'T') {
            $d->setOcorrencia($this->rem(16, 17, $detalhe))
                ->setOcorrenciaDescricao(Util::array_get($this->ocorrencias, $this->detalheAtual()->getOcorrencia(), 'Desconhecida'))
            ...
            */
            ...
            if ($d->hasOcorrencia('06', '17')) {
                ...
                $ocorrencia = Util::appendStrings(
                    ...
                    Util::array_get($this->baixa_liquidacao, $msgAdicional[0], ''),
                    Util::array_get($this->baixa_liquidacao, $msgAdicional[1], ''),
                    Util::array_get($this->baixa_liquidacao, $msgAdicional[2], ''),
                    Util::array_get($this->baixa_liquidacao, $msgAdicional[3], ''),
                    Util::array_get($this->baixa_liquidacao, $msgAdicional[4], '')
                );
                ...
                $ocorrencia = Util::appendStrings(
                    $d->getOcorrenciaDescricao(),
                    Util::array_get($this->rejeicoes, $msgAdicional[0], ''),
                    Util::array_get($this->rejeicoes, $msgAdicional[1], ''),
                    Util::array_get($this->rejeicoes, $msgAdicional[2], ''),
                    Util::array_get($this->rejeicoes, $msgAdicional[3], ''),
                    Util::array_get($this->rejeicoes, $msgAdicional[4], '')
                );
                $d->setOcorrenciaDescricao($ocorrencia);
